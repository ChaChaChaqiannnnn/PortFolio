...
function WebGLWorld() {
  ...
  useEffect(() => {
    ...
    const animate = () => { const t = clock.getElapsedTime(); group.rotation.y += (pointerX * 0.28 - group.rotation.y) * 0.025; group.rotation.x += (-pointerY * 0.18 - group.rotation.x) * 0.025; if (!reduced) { objects[0].rotation.z = t * 0.14; objects[0].rotation.y = t * 0.1; objects[1].position.y = 1.35 + Math.sin(t * 0.8) * 0.18; objects[2].rotation.x = t * -0.28 } renderer.render(scene, camera); frame = requestAnimationFrame(animate) };
    resize(); animate(); mount.addEventListener("pointermove", onPointerMove); window.addEventListener("resize", resize);
    return () => { cancelAnimationFrame(frame); mount.removeEventListener("pointermove", onPointerMove); window.removeEventListener("resize", resize); geometries.forEach((g) => g.dispose()); dotGeometry.dispose(); scene.traverse((child) => { if (child instanceof THREE.Mesh && child.material instanceof THREE.Material) child.material.dispose() }); renderer.dispose(); renderer.domElement.remove() };
  }, []);
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="work-section" id="work"><div className="section-heading"><p className="eyebrow"><Sparkles size={15} /> Selected work</p><h2>Ideas, shipped.</h2><p>A growing archive of experiments, products and beautifully odd things for the web.</p></div><div className="filter-row" aria-label="Filter projects">{filters.map((filter) => <button key={filter} className={activeFilter === filter ? "active" : ""} onClick={() => setActiveFilter(filter)}>{filter}</button>)}</div><div className="project-list">{visibleProjects.map((project) => <button className="project-row" key={project.id} onClick={() => setSelectedProject(project)} style={{ "--project-color": project.color } as React.CSSProperties}><span className="project-index">{project.index}</span><span className="project-orb" /><span className="project-title">{project.title}</span><span className="project-meta">{project.category} · {project.year}</span><span className="project-arrow"><ArrowUpRight size={24} /></span></button>)}</div></section>
    <section className="about-section" id="about"><p className="eyebrow"><Asterisk size={15} /> About</p><div className="about-grid"><h2>Part engineer,<br />part daydreamer.</h2><div><p>I turn ambitious ideas into expressive, performant experiences. My sweet spot is where precise engineering meets a little visual mischief.</p><div className="skills"><span>Creative direction</span><span>Frontend systems</span><span>WebGL / shaders</span><span>Product design</span></div></div></div></section>
